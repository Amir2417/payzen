...
class ReceiverRecipientController extends Controller
{
    ...
    {
        $page_title = "My Receiver Recipient";
    ...
    public function storeReceipient(Request $request){
        ...
        $request->validate([
            ...
            $checkMobile = AgentRecipient::auth()->receiver()->where('mobile',$request->mobile)->first();
            if($checkMobile){
        ...
        try{
            ...
            return redirect()->route('agent.receiver.recipient.index')->with(['success' => ['Receiver recipient save successfully']]);
        }catch(Exception $e) {
    ...
    public function updateReceipient(Request $request){
        ...
        $checkMobile = AgentRecipient::where('id','!=',$data->id)->auth()->receiver()->where('mobile',$request->mobile)->first();
        ...
        try{
            $data->fill($in)->save();
            return redirect()->route('agent.receiver.recipient.index')->with(['success' => ['Receiver recipient updated successfully']]);
        }catch(Exception $e) {
            return back()->with(['error' => ["Something is wrong"]]);
        }
    ...
    public function deleteReceipient(Request $request) {
        ...
        return back()->with(['success' => ['Receiver recipient deleted successfully!']]);
    }
    //get dynamic fields
