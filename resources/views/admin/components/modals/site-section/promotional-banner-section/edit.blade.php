<div id="promotional-banner-edit" class="mfp-hide large">
    <div class="modal-data">
        <div class="modal-header px-0">
            <h5 class="modal-title">{{ __("Edit Promotional Banner") }}</h5>
        </div>
                            ...
                            <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif" id="modal-english" role="tabpanel" aria-labelledby="modal-english-tab">
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        'label'     => "Button Name*",
                                        'name'      => $default_lang_code . "_button_name_edit",
                                        'value'     => old($default_lang_code . "_button_name_edit",$data->value->language->$default_lang_code->button_name ?? "")
                                    ])
                                </div>
                            </div>
                            @foreach ($languages as $item)
                                ...
                                <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="modal-{{ $item->name }}" role="tabpanel" aria-labelledby="modal-{{$item->name}}-tab">
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            ...
                                            'value'     => old($lang_code . "_button_name_edit",$data->value->language->$lang_code->button_name ?? "")
                                        ])
                                    </div>
                                </div>
                            @endforeach
