                        ...
                        <form class="card-form" action="{{ setRoute('agent.bill.pay.confirm') }}" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-6 col-lg-6  form-group">
                                    ...
                                    <select name="bill_type" class="form--control select2-basic bill-type-list" required data-placeholder="Bill Type" >

                                    </select>
                                </div>
                                <div class="col-xl-6 col-lg-6  form-group">
                                    <label  class="customer-identifier">{{ __("Customer Identifier") }} <span class="text--base">*</span></label>
                                    <input type="text" class="form--control" required name="customer_identifier" placeholder="Enter Customer Identifier" value="{{ old('customer_identifier') }}">
...
<script>
   ...
   $(document).ready(function(){
    ...
    $("input[name=customer_identifier]").keyup(function(){
        getFees();
        getPreview();
    });
    ...
    function acceptVar() {
        ...
        var billNumber = $("input[name=customer_identifier]").val();
        var customer_identifier = $("select[name=bill_type] :selected").data("label");;
    ...
    function getPreview() {
            ...
            var customerIdentifier = acceptVar().customerIdentifier;
            senderAmount == "" ? senderAmount = 0 : senderAmount = senderAmount;
            // Sending Amount
            $('.request-amount').text(senderAmount + " " + defualCurrency);
            //bill type
            $('.bill-type').text(billName);
            $('.customer-identifier').text(customerIdentifier);
            // Fees
            //bill number
    ...
    if( sender_amount < min_limit ){
     ...
     function  getBillTypeList(){
        ...
        $(".bill-type-list").html('');
        $.ajax({
                url: "{{route('agent.bill.pay.types')}}",
                type: "POST",
                data: {
                ...
                success: function (res) {
                    ...
                    $.each(list.data, function (key, value) {
                        $(".bill-type-list").append('<option value="' + value.biller_name + '" data-name="' + value.name + '" data-label="' + value.label_name + '" data-amount="' + value.amount + '" ' + ' >' + value.biller_name + '</option>');
                    });
            }
        });
